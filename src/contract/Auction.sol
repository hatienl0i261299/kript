...
contract Auction {
    ...
    function transfer(uint auctionID) public payable {
        AuctionStruct memory auction = auctions[auctionID];
        auction.isEnded = true;
        emit transferEvent(auction.highestBid, auction.owner, auction.amount, auction.image, auction.info, auction.endTime);
    }

    function getAuctionByID(uint auctionID) public view returns (AuctionStruct memory) {
        AuctionStruct memory auction = auctions[auctionID];
        if (auction.isEnded) {
            revert AuctionAlreadyEnded();
        }
